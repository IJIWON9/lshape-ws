...
from models.cnn1d import CNN1DClassifier  # 👈 반드시 모델 정의 있어야 함

class CNN1DInferenceNode(Node):
    def __init__(self):
        super().__init__('contour_cnn1d_inference_node')
        ...
        model_path = os.path.join(package_path, 'weights', 'cnn1d_model.pth')

        self.model = CNN1DClassifier(input_length=95, num_classes=2)
    ...
    def listener_callback(self, msg):
        for contour_idx, segment in enumerate(msg.contours):
            for seg_idx, pc_msg in enumerate(segment.contour_segment):
                ...
                # CNN 입력: 정규화 후 [1, 1, L] shape
                vec = feature * 10.0
                vec = (vec - np.mean(vec)) / (np.std(vec) + 1e-6)
                input_tensor = torch.tensor(vec, dtype=torch.float32).unsqueeze(0)
