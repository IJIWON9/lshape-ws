...
struct Params 
{
    ...
    Params() {
        ...
        sensor_height = 1.723;     // skku ioniq : 1m + halfwheel  
        ...
        max_range = 120.0;           // max_range of ground estimation area
...
void PatchWorkpp::flush_patches(vector<Zone> &czm) {
    ...
    if( params_.verbose ) cout << "\033[1;32m" << "PatchWorkpp::flush_patches() - Flushed patches successfully!" << "\033[0m" << endl;
}
